...
import com.juniorsilvacc.erudio.exceptions.InvalidJwtAuthenticationException;
...
public class JwtTokenProvider {
	...
	public TokenDTO	refrashToken (String refrashToken) {
		if(refrashToken.contains("Bearer ")) {
			refrashToken = refrashToken.substring("Bearer ".length());
		}
		
		JWTVerifier verifier = JWT.require(algorithm).build();
		DecodedJWT decodedJWT = verifier.verify(refrashToken);
	...
	private String getRefreshToken(String username, List<String> roles, Date now) {
		Date validityRefreshToken = new Date(now.getTime() + (validatyInMilliSeconds * 3));
		return JWT.create()
				.withClaim("roles", roles)
				.withIssuedAt(now)
